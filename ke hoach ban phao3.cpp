...
void main()
{
	textmode(C4350);
   _setcursortype(0);
	int star[100],star1[100],k[100],star2[100],star3[100],k1[100],spt1=1,spt=1,star4[100],star5[100],k2[100],spt2=1;
	int x,y;
   start_star(x,y);
   display_star(x,y,15,14);
   start_star(star,star1,k,spt,x,y);
   start_star(star2,star3,k1,spt1,x,y);
   start_star(star4,star5,k2,spt2,x,y);
   display_star(star,star1,spt);
   clock_t start,end,lower,super_lower;
   super_lower=clock();
   start=clock();
   lower=clock();
   int lock;
   randomize();
   bool co=true,co1=true,co2=true,co3=true;
   while(1)
   {
   	end=clock();
      if(((end-start)/CLK_TCK)>=.1)
      {
         ...
         if(!move_type_one(star,star1,k,spt,-1,12,true))
         {
         	del_display_star(star4,star5,spt2);
               del_display_star(star,star1,spt);
         ...
         if(((end-lower)/CLK_TCK)>=.4)
      	{
           ...
           if(((end-super_lower)/CLK_TCK)>=.7)
           {
					if(co3)
            {
         	...
         	del_display_star(star4,star5,spt2);
         	if(!move_type_one(star4,star5,k2,spt2,-1,12,co2))
            {
            	co3=true;
            	del_display_star(star4,star5,spt2);
               del_display_star(star,star1,spt);
